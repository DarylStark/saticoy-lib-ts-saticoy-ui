...
} from '../../selectable-item-menu/selectable-item-menu';
...
function ThemeSelectMenu({
    ...
    const onChangeMode = (new_mode: string | string[]) => {
        if (Array.isArray(new_mode)) return props.onChangeMode?.(new_mode[0]);
        return props.onChangeMode?.(new_mode);
    };

    const onChangeTheme = (new_mode: string | string[]) => {
        if (Array.isArray(new_mode)) return props.onChangeTheme?.(new_mode[0]);
    ...
    return (
        <UserSelectableItemMenu icon={buttonIcons[props.selectedMode]}>
                ...
                <>
                    <SelectableItemMenu
                        ...
                        items={[
                            {
                                value: 'auto',
                                name: props.stringAutomaticMode,
                                isDiabled: !modeSelectorEnabled,
                            },
                            {
                                value: 'dark',
                                name: props.stringDarkMode,
                                isDiabled: !modeSelectorEnabled,
                            },
                            {
                                value: 'light',
                                name: props.stringLightMode,
                                isDiabled: !modeSelectorEnabled,
                            },
                        ]}
                    />
            ...
            {showModeSelector && showThemeSelector && <MenuDivider />}
            {showThemeSelector && (
                <SelectableItemMenu
